...
process.on('unhandledRejection', reason => {
  // oxlint-disable-next-line no-console -- process-level safety net runs outside the request-scoped logger
...
const shutdown = async (signal: string): Promise<void> => {
  // oxlint-disable-next-line no-console -- bootstrap-time stdout before logger is wired into systemd
