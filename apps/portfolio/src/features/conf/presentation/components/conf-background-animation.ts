 ...
 * connected by faint lines. Each node emits two outward ripples, like voice
 * pulses leaving a mouthpiece. ≤18 nodes on desktop, halved on low-core
 * devices. Pure animation logic; the React shell lives in `ConfBackground.tsx`.
 */
...
export function createConfBackgroundAnimation(): IAmbientCanvasAnimation {
  let nodes: INode[] = [];
  let colorPrimary = '#a78bfa';
  let colorAccent = '#f5c842';

  return {
    onResize({ ctx, cssWidth, cssHeight, dpr }: IAmbientCanvasResize): void {
      // Sizing the canvas reset the 2D context — re-apply the DPR transform so
      // the draw works in CSS pixels. Theme colours + node layout depend on
      // size, so (re)read them here rather than per frame.
      ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
      colorPrimary = readCssColor(VAR_PURPLE, '#a78bfa');
      colorAccent = readCssColor(VAR_YELLOW, '#f5c842');
      nodes = createNodes(resolveNodeCount(cssWidth, cssHeight), performance.now());
    },

    draw(frame: IAmbientCanvasFrame): void {
      const elapsedSec = frame.elapsedMs / MS_PER_SECOND;
      drawFrame({
        ctx: frame.ctx,
      ...
      const fadeIn = frame.isStatic ? 1 : Math.min(1, elapsedSec / FADE_IN_DURATION_SEC);
      frame.ctx.canvas.style.opacity = `${fadeIn * DEFAULT_OPACITY}`;
    },
  };
}
