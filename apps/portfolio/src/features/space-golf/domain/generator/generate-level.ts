 ...
 * plain faces, spike rows on a few others. Deterministic — the same seed
 * gives the same level everywhere. Nothing yet proves the cup reachable (§9).
 */
export function generateLevel(seed: number): Level {
  ...
  return { ...carved, spikes: placeSpikes(random, carved) };
}
