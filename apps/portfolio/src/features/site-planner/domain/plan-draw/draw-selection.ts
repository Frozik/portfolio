...
} from '../geometry/polygonize-shape';
import { anchorPlanPosition } from '../geometry/shape-anchor';
import type { RectangleHandleFactors } from '../geometry/transform-shape';
import type { CircleShape, RectangleShape, Shape } from '../model/shapes';
import type { PlanViewport } from '../view/plan-viewport';
import { planDirectionToScreen, planToScreen } from '../view/plan-viewport';
...
export function computeShapeHandles(shape: Shape, viewport: PlanViewport): readonly ShapeHandle[] {
  switch (shape.kind) {
    case 'rectangle':
      return computeRectangleHandles(shape, viewport);
    case 'circle':
...
function computeRectangleHandles(
 ...
 */
function offsetAlongLocalNorth(rectangle: RectangleShape, topScreenPoint: Vector2): Vector2 {
