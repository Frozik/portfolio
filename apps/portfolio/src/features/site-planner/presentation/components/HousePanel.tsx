...
import type { BuildingScene, SitePlannerStore } from '../../application/SitePlannerStore';
import type { FoundationKind, UtilityEntry, UtilitySystem } from '../../domain/model/foundation';
import { ENTRY_SYSTEMS, FOUNDATION_KINDS } from '../../domain/model/foundation';
import type { PadElevationMode } from '../../domain/model/site-plan';
import { entriesOf, PAD_ELEVATION_MODES } from '../../domain/model/site-plan';
      ...
      <div className="flex flex-col gap-1 rounded-md border border-white/10 p-1.5">
        <div className="flex items-center gap-1.5">
          ...
          <span className="min-w-0 flex-1 truncate text-[11px] text-text">
            ...
            <span className="text-text-secondary"> · {labels.kinds[entry.kind]}</span>
          </span>
          <button
            type="button"
 ...
 * site trench and an indoor route will meet at. Panel-driven until the
 * building editor's own entry tool ships with stage 2.
 */
const EntriesBlock = observer(
  ({ store, scene }: { readonly store: SitePlannerStore; readonly scene: BuildingScene }) => {
    const labels = sitePlannerT.house.entries;
