...
function getPoints(scores: ReturnType<typeof scoreRound>, playerId: number): number {
...
describe('scoreRound', () => {
  it('scores damage continuously as it is dealt', () => {
    const scores = scoreRound(
      ...
      createOutcome({
        damages: [
          { dealerId: 1, targetId: 2, amount: 30 },
          { dealerId: 1, targetId: 3, amount: 12 },
        ],
      })
    );

    expect(getPoints(scores, 1)).toBe(42);
    expect(getPoints(scores, 2)).toBe(0);
  });
  ...
  it('subtracts damage a player deals to itself', () => {
    const scores = scoreRound(
      ...
      createOutcome({
        damages: [
          { dealerId: 1, targetId: 2, amount: 30 },
          { dealerId: 1, targetId: 1, amount: 20 },
        ],
      })
    );

    expect(getPoints(scores, 1)).toBe(10);
  });

  it('ignores damage nobody caused', () => {
    const scores = scoreRound(
      PLAYER_IDS,
      createOutcome({ damages: [{ dealerId: undefined, targetId: 2, amount: 40 }] })
    );
  ...
  it('adds the kill bonus on top of the damage points', () => {
    const scores = scoreRound(
      PLAYER_IDS,
      createOutcome({
        damages: [{ dealerId: 1, targetId: 2, amount: 100 }],
        kills: [{ killerId: 1, victimId: 2 }],
      })
    );

    expect(getPoints(scores, 1)).toBe(100 + STANDARD_KILL_POINTS);
  });

  it('charges a player the bonus for killing itself', () => {
    const scores = scoreRound(PLAYER_IDS, createOutcome({ kills: [{ killerId: 1, victimId: 1 }] }));

    expect(getPoints(scores, 1)).toBe(-STANDARD_KILL_POINTS);
  });
  ...
  it('lets a retreat forfeit the points but deny the killer their bounty', () => {
    const scores = scoreRound(
      ...
      createOutcome({
        damages: [
          { dealerId: 2, targetId: 3, amount: 50 },
          { dealerId: 1, targetId: 2, amount: 40 },
        ],
        kills: [{ killerId: 1, victimId: 2 }],
        retreatedIds: [2],
      })
    ...
    expect(getPoints(scores, 2)).toBe(0);
    expect(getPoints(scores, 1)).toBe(40);
  });
  ...
  it('pays out the round winnings at the cash rate and never goes negative', () => {
    const scores = scoreRound(
      ...
      createOutcome({
        damages: [
          { dealerId: 1, targetId: 2, amount: 10 },
          { dealerId: 2, targetId: 2, amount: 30 },
        ],
      })
    );
  ...
  it('counts only kills that were neither self-inflicted nor denied by a retreat', () => {
    const outcome = createOutcome({
      kills: [
        ...
        { killerId: 1, victimId: 3 },
        { killerId: 1, victimId: 1 },
      ],
      retreatedIds: [3],
    });

    expect(countScoringKills(outcome, 1)).toBe(1);
    expect(countScoringKills(outcome, 2)).toBe(0);
  });
});
...
describe('rankStandings', () => {
  it('puts the most kills first and breaks ties on points', () => {
    const ranked = rankStandings([
      { playerId: 1, kills: 2, points: 500 },
      { playerId: 2, kills: 4, points: 100 },
      { playerId: 3, kills: 2, points: 900 },
    ]);
  ...
  it('leaves the input untouched', () => {
    const standings = [
      { playerId: 1, kills: 0, points: 0 },
      { playerId: 2, kills: 5, points: 0 },
    ];
