  ...
  return (
    <section className="flex flex-col gap-3">
      <div className="flex items-center gap-3.5">
        <MonoKicker tone="accent">{retroT.close.exportKicker}</MonoKicker>
        <span className="section-rule" />
      </div>
      <CardFrame className="flex flex-col items-start gap-3 px-5 py-5">
        <h2 className="m-0 text-[15px] font-medium text-landing-fg">{retroT.close.title}</h2>
        <p className="m-0 text-[13px] leading-[1.55] text-landing-fg-dim">
          {retroT.close.summarySubtitle}
        </p>
        <button
          type="button"
