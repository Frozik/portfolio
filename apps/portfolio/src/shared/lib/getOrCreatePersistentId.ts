...
export function getOrCreatePersistentId<TId>(params: {
  ...
  /** Parse a stored raw string into an id, or `undefined` if malformed. */
  readonly parse: (raw: string) => TId | undefined;
  ...
  try {
    ...
    const existing = isNil(raw) ? undefined : params.parse(raw);
    if (!isNil(existing)) {
      return existing;
    }
