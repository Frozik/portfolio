#!/usr/bin/env bash
# remote-run.sh — rsync lib/ + roles/ to target then invoke role steps over SSH.
#
...
#
# Exports: start_ssh_master, stop_ssh_master, rsync_infra_to_target,
...
# rsync_infra_to_target — copy lib/ and roles/ to
# /tmp/communication-install-<TS> on the target host, preserving the layout
# role scripts expect (they source ../../lib/common.sh).
# Idempotent (rsync only re-copies changed files).
rsync_infra_to_target() {
  require_env SSH_HOST
  info "Syncing infra to ${SSH_HOST}:${REMOTE_DIR}"
  ssh_via_master "${SSH_HOST}" "mkdir -p '${REMOTE_DIR}/lib' '${REMOTE_DIR}/roles'"
  for dir in lib roles; do
...
# remote_run_script <role>/<step> — execute roles/<role>/<step>.sh on the
# target host with environment forwarded. Reuses the multiplexed ssh master.
remote_run_script() {
  local step="$1"
  require_env SSH_HOST
  info "Running on ${SSH_HOST}: ${step}"
  # shellcheck disable=SC2029
  ssh_via_master "${SSH_HOST}" "cd '${REMOTE_DIR}' && env $(env_for_remote) bash 'roles/${step}.sh'"
  ok "Done: ${step}"
}
