...
`,H=48/Float32Array.BYTES_PER_ELEMENT,U=class{uniformManager;device;shapesPipeline;shapesBindGroup;shapesStorageBuffer;shapeDataBuffer=z(_);shapes=[];constructor(e){this.uniformManager=e}init(e){this.device=e.device;let t=this.device.createShaderModule({code:V}),n=this.device.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:`uniform`}},{binding:1,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:`read-only-storage`}}]});this.shapesStorageBuffer=this.device.createBuffer({size:_*48,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.shapesPipeline=this.device.createRenderPipeline({layout:this.device.createPipelineLayout({bindGroupLayouts:[n]}),vertex:{module:t,entryPoint:`vsShapes`,constants:{FADE_DURATION:v}},fragment:{module:t,entryPoint:`fsShapes`,targets:[{format:e.format,blend:{color:{srcFactor:`one`,dstFactor:`one-minus-src-alpha`,operation:`add`},alpha:{srcFactor:`one`,dstFactor:`one-minus-src-alpha`,operation:`add`}}}]},primitive:{topology:`triangle-list`}}),this.shapesBindGroup=this.device.createBindGroup({layout:n,entries:[{binding:0,resource:{buffer:this.uniformManager.buffer}},{binding:1,resource:{buffer:this.shapesStorageBuffer}}]})}update(e){let{time:t,canvasWidth:n,canvasHeight:r,devicePixelRatio:i}=e,a=n*m,o=r*m;if(this.shapes.length===0){let e=I(n,r,i);this.shapes=L(e)}let s=I(n,r,i);s!==this.shapes.length&&R(this.shapes,s,t,a,o);for(let e=0;e<this.shapes.length;e++){let n=this.shapes[e];if(t-n.spawnTime>P(n)){let n=N(t);n.x=M(-a+n.halfSize,a-n.halfSize),n.y=M(-o+n.halfSize,o-n.halfSize),this.shapes[e]=n}F(this.shapes[e],this.shapeDataBuffer,e*H)}this.device.queue.writeBuffer(this.shapesStorageBuffer,0,this.shapeDataBuffer.buffer,0,this.shapes.length*48)}render(e,t,n){if(this.shapes.length===0)return;let r=e.beginRenderPass({colorAttachments:[{view:t,loadOp:`load`,storeOp:`store`}]});r.setPipeline(this.shapesPipeline),r.setBindGroup(0,this.shapesBindGroup),r.draw(6,this.shapes.length,0,0),r.end()}dispose(){this.shapesStorageBuffer.destroy()}},W=class{textureManager;compositeBindGroupLayout;compositeSampler;compositeUniformBuffer;chartShaderModule;uniformManager;device;sinYPipeline;bindGroup;constructor(e,t,n,r,i,a){this.textureManager=e,this.compositeBindGroupLayout=t,this.compositeSampler=n,this.compositeUniformBuffer=r,this.chartShaderModule=i,this.uniformManager=a}init(e){this.device=e.device;let t=this.device.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:`uniform`}}]});this.sinYPipeline=this.device.createRenderPipeline({layout:this.device.createPipelineLayout({bindGroupLayouts:[t]}),vertex:{module:this.chartShaderModule,entryPoint:`vsSinY`},fragment:{module:this.chartShaderModule,entryPoint:`fsSinY`,targets:[{format:C,blend:w}]},primitive:{topology:`triangle-list`},multisample:{count:4}}),this.bindGroup=this.device.createBindGroup({layout:t,entries:[{binding:0,resource:{buffer:this.uniformManager.buffer}}]})}update(){}render(e,t,n){let i=S(n.canvasHeight),a=this.textureManager.ensureOffscreenTextures(n.canvasWidth,n.canvasHeight,this.compositeBindGroupLayout,this.compositeSampler,this.compositeUniformBuffer);if(r(a))return;let o=e.beginRenderPass({colorAttachments:[{view:a.offscreenMsaaView,resolveTarget:a.offscreenResolveView,loadOp:`clear`,clearValue:{r:0,g:0,b:0,a:0},storeOp:`discard`}]});o.setPipeline(this.sinYPipeline),o.setBindGroup(0,this.bindGroup),i>0&&o.draw(18,i,0,0),o.end()}dispose(){}},G=class{uniformManager;device;constructor(e){this.uniformManager=e}init(e){this.device=e.device}update(e){this.uniformManager.writeFromFrameState(this.device,e)}render(){}dispose(){}},K=`const PI: f32 = 3.14159265358979323846;
const HALF: f32 = 0.5;
...
`;function q(e,t){let n=u(d(t).uniforms.U),r=e.createBuffer({size:n.arrayBuffer.byteLength,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});return{buffer:r,writeFromFrameState(e,t){let i=t.canvasWidth*m,a=t.canvasHeight*m,o=f.ortho(-i,i,-a,a,-1,1),s=x(t.canvasWidth),c=S(t.canvasHeight);n.set({mvp:o,viewport:[t.canvasWidth,t.canvasHeight],time:t.time,sinCount:s,sinPenMin:2,sinPenMax:20,borderMargin:20,borderOffset:s,sinYCount:c}),e.queue.writeBuffer(r,0,n.arrayBuffer)},dispose(){r.destroy()}}}var J=B+K;function Y(e){let t=!1,n;return X(e).then(e=>{t?e():n=e},e=>{console.error(`Failed to initialize charts renderer`,e)}),()=>{t=!0,n?.()}}async function X(e){let t=await c(e),{device:n}=t,r=n.createShaderModule({code:J}),i=T(n,C,4),a=o(4),u=k(n),d=q(n,B),f=new s([new G(d),new j(r,a,d),new W(i,u.compositeBindGroupLayout,u.compositeSampler,u.compositeUniformBuffer,r,d),new A(i,u),new U(d)]);f.initAll(t);let p=l({canvas:e,context:t,layerManager:f});return()=>{p(),f.dispose(),d.dispose(),a.dispose(),i.destroy(),n.destroy()}}var Z=t(),Q=(0,p.memo)(()=>{let e=(0,p.useRef)(null);return(0,p.useEffect)(()=>{if(e.current)return Y(e.current)},[]),(0,Z.jsx)(i,{className:`h-full w-full`,children:(0,Z.jsx)(`canvas`,{ref:e,className:`h-full w-full`})})});export{Q as Charts};