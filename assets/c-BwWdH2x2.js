import{M as e,N as t,P as n,st as r}from"./c-Cy9pq3c4.js";import{i,r as a,t as o}from"./c-76e03v0C.js";import{n as s,r as c,t as l}from"./c-DMWsDBIV.js";import{t as u}from"./c-wJEgkeeX.js";var d=r(t(),1),f=.149,p=.149,m=.149,h=.5,g=.7,_=1e-4,v=16384,y=.5,b=.4,x=.6,S=`rgba8unorm`,C=1e3;function w(e){return Math.trunc(e/20/4)*4+1}function T(e){return Math.trunc(e/20/4)*4+1}var E={color:{srcFactor:`src-alpha`,dstFactor:`one-minus-src-alpha`,operation:`add`},alpha:{srcFactor:`one`,dstFactor:`one-minus-src-alpha`,operation:`add`}};function D(e,t,r){let i=null,a=null,o=null,s=null,c=null;return{ensureOffscreenTextures(l,u,d,f,p){return!n(i)&&i.width===l&&i.height===u?n(a)||n(s)||n(c)?null:{offscreenMsaaView:a,offscreenResolveView:s,compositeBindGroup:c}:(i?.destroy(),o?.destroy(),l===0||u===0?(i=null,a=null,o=null,s=null,c=null,null):(i=e.createTexture({size:[l,u],format:t,sampleCount:r,usage:GPUTextureUsage.RENDER_ATTACHMENT}),a=i.createView(),o=e.createTexture({size:[l,u],format:t,sampleCount:1,usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.TEXTURE_BINDING}),s=o.createView(),c=e.createBindGroup({layout:d,entries:[{binding:0,resource:s},{binding:1,resource:f},{binding:2,resource:{buffer:p}}]}),{offscreenMsaaView:a,offscreenResolveView:s,compositeBindGroup:c}))},destroy(){i?.destroy(),o?.destroy()}}}var O=`struct CompositeUniforms {
    opacity: f32,
};
...
`,k=3,A=16;function j(e){let t=e.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:`float`}},{binding:1,visibility:GPUShaderStage.FRAGMENT,sampler:{type:`filtering`}},{binding:2,visibility:GPUShaderStage.FRAGMENT,buffer:{type:`uniform`}}]}),n=e.createSampler({magFilter:`linear`,minFilter:`linear`}),r=e.createBuffer({size:A,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),i=new Float32Array([g,0,0,0]);return e.queue.writeBuffer(r,0,i),{compositeBindGroupLayout:t,compositeSampler:n,compositeUniformBuffer:r}}var M=class{compositePipeline;constructor(e,t){this.textureManager=e,this.resources=t}init(e){let{device:t,format:n}=e,r=t.createShaderModule({code:O});this.compositePipeline=t.createRenderPipeline({layout:t.createPipelineLayout({bindGroupLayouts:[this.resources.compositeBindGroupLayout]}),vertex:{module:r,entryPoint:`vsComposite`},fragment:{module:r,entryPoint:`fsComposite`,targets:[{format:n,blend:E}]},primitive:{topology:`triangle-list`}})}update(e){}render(e,t,r){let i=this.textureManager.ensureOffscreenTextures(r.canvasWidth,r.canvasHeight,this.resources.compositeBindGroupLayout,this.resources.compositeSampler,this.resources.compositeUniformBuffer);if(n(i))return;let a=e.beginRenderPass({colorAttachments:[{view:t,loadOp:`load`,storeOp:`store`}]});a.setPipeline(this.compositePipeline),a.setBindGroup(0,i.compositeBindGroup),a.draw(k,1,0,0),a.end()}dispose(){this.resources.compositeUniformBuffer.destroy()}},N=`struct Uniforms {
    mvp: mat4x4<f32>,
    viewport: vec2<f32>,
    time: f32,
    sinCount: u32,
...
`;function P(e,t){let n=s(l(t).uniforms.U),r=e.createBuffer({size:n.arrayBuffer.byteLength,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});return{buffer:r,writeFromFrameState(e,t){let i=t.canvasWidth*h,a=t.canvasHeight*h,s=o.ortho(-i,i,-a,a,-1,1),c=w(t.canvasWidth),l=T(t.canvasHeight);n.set({mvp:s,viewport:[t.canvasWidth,t.canvasHeight],time:t.time,sinCount:c,sinPenMin:2,sinPenMax:20,borderMargin:20,borderOffset:c,sinYCount:l}),e.queue.writeBuffer(r,0,n.arrayBuffer)},dispose(){r.destroy()}}}var F=class{device;format;uniformManager;pipeline;bindGroup;constructor(e,t){this.chartShaderModule=e,this.msaaManager=t}init(e){this.device=e.device,this.format=e.format,this.uniformManager=P(this.device,N);let t=this.device.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:`uniform`}}]});this.pipeline=this.device.createRenderPipeline({layout:this.device.createPipelineLayout({bindGroupLayouts:[t]}),vertex:{module:this.chartShaderModule,entryPoint:`vs`},fragment:{module:this.chartShaderModule,entryPoint:`fs`,targets:[{format:this.format,blend:E}]},primitive:{topology:`triangle-list`},multisample:{count:4}}),this.bindGroup=this.device.createBindGroup({layout:t,entries:[{binding:0,resource:{buffer:this.uniformManager.buffer}}]})}update(e){this.uniformManager.writeFromFrameState(this.device,e)}render(e,t,r){let i=w(r.canvasWidth)+4,a=this.msaaManager.ensureView(this.device,this.format,r.canvasWidth,r.canvasHeight);if(n(a))return;let o=e.beginRenderPass({colorAttachments:[{view:a,resolveTarget:t,loadOp:`clear`,clearValue:{r:f,g:p,b:m,a:1},storeOp:`discard`}]});o.setPipeline(this.pipeline),o.setBindGroup(0,this.bindGroup),i>0&&o.draw(18,i,0,0),o.end()}dispose(){this.uniformManager.dispose()}};function I(e,t){return e+Math.random()*(t-e)}function L(e){let t=Math.random(),n=Math.random(),r=Math.random(),i=(t+n+r)/3;if(i<.4){let e=b/Math.max(i,.01);t=Math.min(1,t*e),n=Math.min(1,n*e),r=Math.min(1,r*e)}return{x:0,y:0,halfSize:I(40/2,160/2),spawnTime:e,r:t,g:n,b:r,holdDuration:I(2,3),shapeType:Math.floor(Math.random()*10),fillMode:Math.random()<.5?0:1,maxOpacity:I(x,1)}}function R(e){return 2*y+e.holdDuration}function z(e,t,n){t[n]=e.x,t[n+1]=e.y,t[n+2]=e.halfSize,t[n+3]=e.spawnTime,t[n+4]=e.r,t[n+4+1]=e.g,t[n+4+2]=e.b,t[n+4+3]=e.holdDuration,t[n+8]=e.shapeType,t[n+8+1]=e.fillMode,t[n+8+2]=e.maxOpacity,t[n+8+3]=0}function B(e,t,n){let r=e/n*(t/n);return Math.min(Math.max(1,Math.round(r*_)),v)}function V(e){let t=[],n=2*y+5/2;for(let r=0;r<e;r++){let i=L(0);i.spawnTime=-(n/e)*r,t.push(i)}return t}function H(e,t,n,r,i){if(t>e.length){let a=2*y+5/2,o=t-e.length;for(let t=0;t<o;t++){let s=L(n);s.spawnTime=n-a/o*t,s.x=I(-r+s.halfSize,r-s.halfSize),s.y=I(-i+s.halfSize,i-s.halfSize),e.push(s)}}else t<e.length&&e.splice(t)}function U(e){return new Float32Array(e*48/Float32Array.BYTES_PER_ELEMENT)}var W=N+`const HALF: f32 = 0.5;
...
`,G=48/Float32Array.BYTES_PER_ELEMENT,K=class{device;uniformManager;shapesPipeline;shapesBindGroup;shapesStorageBuffer;shapeDataBuffer=U(v);shapes=[];init(e){this.device=e.device,this.uniformManager=P(this.device,W);let t=this.device.createShaderModule({code:W}),n=this.device.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:`uniform`}},{binding:1,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:`read-only-storage`}}]});this.shapesStorageBuffer=this.device.createBuffer({size:v*48,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.shapesPipeline=this.device.createRenderPipeline({layout:this.device.createPipelineLayout({bindGroupLayouts:[n]}),vertex:{module:t,entryPoint:`vsShapes`},fragment:{module:t,entryPoint:`fsShapes`,targets:[{format:e.format,blend:{color:{srcFactor:`one`,dstFactor:`one-minus-src-alpha`,operation:`add`},alpha:{srcFactor:`one`,dstFactor:`one-minus-src-alpha`,operation:`add`}}}]},primitive:{topology:`triangle-list`}}),this.shapesBindGroup=this.device.createBindGroup({layout:n,entries:[{binding:0,resource:{buffer:this.uniformManager.buffer}},{binding:1,resource:{buffer:this.shapesStorageBuffer}}]})}update(e){this.uniformManager.writeFromFrameState(this.device,e);let{time:t,canvasWidth:n,canvasHeight:r,devicePixelRatio:i}=e,a=n*h,o=r*h;this.shapes.length===0&&(this.shapes=V(B(n,r,i)));let s=B(n,r,i);s!==this.shapes.length&&H(this.shapes,s,t,a,o);for(let e=0;e<this.shapes.length;e++){let n=this.shapes[e];if(t-n.spawnTime>R(n)){let n=L(t);n.x=I(-a+n.halfSize,a-n.halfSize),n.y=I(-o+n.halfSize,o-n.halfSize),this.shapes[e]=n}z(this.shapes[e],this.shapeDataBuffer,e*G)}this.device.queue.writeBuffer(this.shapesStorageBuffer,0,this.shapeDataBuffer.buffer,0,this.shapes.length*48)}render(e,t,n){if(this.shapes.length===0)return;let r=e.beginRenderPass({colorAttachments:[{view:t,loadOp:`load`,storeOp:`store`}]});r.setPipeline(this.shapesPipeline),r.setBindGroup(0,this.shapesBindGroup),r.draw(6,this.shapes.length,0,0),r.end()}dispose(){this.uniformManager.dispose(),this.shapesStorageBuffer.destroy()}},q=class{device;uniformManager;sinYPipeline;bindGroup;constructor(e,t,n,r,i){this.textureManager=e,this.compositeBindGroupLayout=t,this.compositeSampler=n,this.compositeUniformBuffer=r,this.chartShaderModule=i}init(e){this.device=e.device,this.uniformManager=P(this.device,N);let t=this.device.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:`uniform`}}]});this.sinYPipeline=this.device.createRenderPipeline({layout:this.device.createPipelineLayout({bindGroupLayouts:[t]}),vertex:{module:this.chartShaderModule,entryPoint:`vsSinY`},fragment:{module:this.chartShaderModule,entryPoint:`fsSinY`,targets:[{format:S,blend:E}]},primitive:{topology:`triangle-list`},multisample:{count:4}}),this.bindGroup=this.device.createBindGroup({layout:t,entries:[{binding:0,resource:{buffer:this.uniformManager.buffer}}]})}update(e){this.uniformManager.writeFromFrameState(this.device,e)}render(e,t,r){let i=T(r.canvasHeight),a=this.textureManager.ensureOffscreenTextures(r.canvasWidth,r.canvasHeight,this.compositeBindGroupLayout,this.compositeSampler,this.compositeUniformBuffer);if(n(a))return;let o=e.beginRenderPass({colorAttachments:[{view:a.offscreenMsaaView,resolveTarget:a.offscreenResolveView,loadOp:`clear`,clearValue:{r:0,g:0,b:0,a:0},storeOp:`discard`}]});o.setPipeline(this.sinYPipeline),o.setBindGroup(0,this.bindGroup),i>0&&o.draw(18,i,0,0),o.end()}dispose(){this.uniformManager.dispose()}};function J(e){let{canvas:t,context:n,layerManager:r}=e,{device:i,canvasContext:a}=n,o=0,s=0,c=Math.max(1,window.devicePixelRatio);function l(){c=Math.max(1,window.devicePixelRatio);let e=Math.floor(t.clientWidth*c),n=Math.floor(t.clientHeight*c);(t.width!==e||t.height!==n)&&(t.width=e,t.height=n),o=e,s=n}l();let u=new ResizeObserver(()=>{l()});u.observe(t);let d=0,f=!1,p=performance.now();function m(){if(f)return;if(l(),o===0||s===0){d=requestAnimationFrame(m);return}let e={time:(performance.now()-p)/C,canvasWidth:o,canvasHeight:s,devicePixelRatio:c};r.updateAll(e);let t=a.getCurrentTexture().createView(),n=i.createCommandEncoder();r.renderAll(n,t,e),i.queue.submit([n.finish()]),d=requestAnimationFrame(m)}return d=requestAnimationFrame(m),()=>{f=!0,cancelAnimationFrame(d),u.disconnect()}}var Y=N+`const PI: f32 = 3.14159265358979323846;
const HALF: f32 = 0.5;
...
`;function X(e){let t=!1,n;return Z(e).then(e=>{t?e():n=e}),()=>{t=!0,n?.()}}async function Z(e){let t=await i(e),{device:n}=t,r=n.createShaderModule({code:Y}),o=D(n,S,4),s=c(4),l=j(n),u=new a([new F(r,s),new q(o,l.compositeBindGroupLayout,l.compositeSampler,l.compositeUniformBuffer,r),new M(o,l),new K]);u.initAll(t);let d=J({canvas:e,context:t,layerManager:u});return()=>{d(),u.dispose(),s.dispose(),o.destroy(),n.destroy()}}var Q=e(),$=(0,d.memo)(()=>{let e=(0,d.useRef)(null);return(0,d.useEffect)(()=>{if(e.current)return X(e.current)},[]),(0,Q.jsx)(`div`,{className:u.fixedContainer,children:(0,Q.jsx)(`canvas`,{ref:e,style:{width:`100%`,height:`100%`}})})});export{$ as Charts};