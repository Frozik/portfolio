import{J as e,X as t,Y as n,i as r,o as i,r as a,wt as o,z as s}from"./c-BUiHm-Fm.js";import{t as c}from"./c-CzYv-hUz.js";import{n as l,t as u}from"./c-B-UvccR9.js";import{n as d,t as f}from"./c-B5wHFoBZ.js";var p=o(n(),1),m=`const PI: f32 = 3.1415926535;
...
`,h=1e6,g={r:.02745,g:.03529,b:.04706,a:1},_=.005,v=.01,y=Math.PI/2,b=.95,x=Math.PI/4,S=.1,C=`depth24plus`,w=1,T=class{device;format;pipeline;bindGroup;uniformBuffer;uniformView;depthTexture=null;constructor(e,t){this.camera=e,this.msaaManager=t}init(e){this.device=e.device,this.format=e.format,this.uniformView=l(u(m).uniforms.U),this.uniformBuffer=this.device.createBuffer({size:this.uniformView.arrayBuffer.byteLength,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});let t=this.device.createShaderModule({code:m}),n=this.device.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:`uniform`}}]});this.pipeline=this.device.createRenderPipeline({layout:this.device.createPipelineLayout({bindGroupLayouts:[n]}),vertex:{module:t,entryPoint:`vs`},fragment:{module:t,entryPoint:`fs`,targets:[{format:this.format}]},primitive:{topology:`triangle-list`,cullMode:`none`},depthStencil:{depthWriteEnabled:!0,depthCompare:`less`,format:C},multisample:{count:4}}),this.bindGroup=this.device.createBindGroup({layout:n,entries:[{binding:0,resource:{buffer:this.uniformBuffer}}]})}update(e){this.camera.tick();let t=this.camera.getViewMatrix(),n=e.canvasWidth/Math.max(w,e.canvasHeight),r=f.perspective(x,n,S,100),i=f.multiply(r,t);this.uniformView.set({time:e.time,mvp:i}),this.device.queue.writeBuffer(this.uniformBuffer,0,this.uniformView.arrayBuffer)}render(e,n,r){let i=this.msaaManager.ensureView(this.device,this.format,r.canvasWidth,r.canvasHeight);if(t(i))return;let a=this.ensureDepthTexture(r.canvasWidth,r.canvasHeight),o=e.beginRenderPass({colorAttachments:[{view:i,resolveTarget:n,loadOp:`clear`,clearValue:g,storeOp:`discard`}],depthStencilAttachment:{view:a.createView(),depthClearValue:1,depthLoadOp:`clear`,depthStoreOp:`discard`}});o.setPipeline(this.pipeline),o.setBindGroup(0,this.bindGroup),o.draw(3,h,0,0),o.end()}dispose(){this.uniformBuffer.destroy(),this.depthTexture?.destroy()}ensureDepthTexture(e,n){return!t(this.depthTexture)&&this.depthTexture.width===e&&this.depthTexture.height===n?this.depthTexture:(this.depthTexture?.destroy(),this.depthTexture=this.device.createTexture({size:[Math.max(w,e),Math.max(w,n)],format:C,sampleCount:4,usage:GPUTextureUsage.RENDER_ATTACHMENT}),this.depthTexture)}};function E(e){let{canvas:t,context:n,layerManager:r}=e,{device:i,canvasContext:a}=n,o=0,c=0,l=Math.max(1,window.devicePixelRatio);function u(){l=Math.max(1,window.devicePixelRatio);let e=Math.floor(t.clientWidth*l),n=Math.floor(t.clientHeight*l);(t.width!==e||t.height!==n)&&(t.width=e,t.height=n),o=e,c=n}u();let d=new ResizeObserver(()=>{u()});d.observe(t);let f=0,p=!1,m=performance.now();function h(){if(p)return;if(u(),o===0||c===0){f=requestAnimationFrame(h);return}let e={time:(performance.now()-m)/s,canvasWidth:o,canvasHeight:c,devicePixelRatio:l};r.updateAll(e);let t=a.getCurrentTexture().createView(),n=i.createCommandEncoder();r.renderAll(n,t,e),i.queue.submit([n.finish()]),f=requestAnimationFrame(h)}return f=requestAnimationFrame(h),()=>{p=!0,cancelAnimationFrame(f),d.disconnect()}}function D(e){let t=16,n=y,r=d.fromValues(Math.sin(n)*Math.sin(0),Math.cos(n),Math.sin(n)*Math.cos(0)),i=d.fromValues(0,1,0);function a(e,t){let n=d.negate(r),a=d.normalize(d.cross(n,i));if(Math.abs(t)>0){let e=-t*_,n=f.rotation(a,e);r=d.transformMat4(r,n),i=d.normalize(d.transformMat4(i,n))}if(Math.abs(e)>0){let t=-e*_,n=f.rotation(i,t);r=d.transformMat4(r,n)}r=d.normalize(r)}function o(e){return Math.max(5,Math.min(20,e))}let s=0,c=0,l=new Map,u=0;function p(){let e=[...l.values()],t=e[0].clientX-e[1].clientX,n=e[0].clientY-e[1].clientY;return Math.sqrt(t*t+n*n)}function m(e){l.set(e.pointerId,{clientX:e.clientX,clientY:e.clientY}),l.size===2&&(u=p())}function h(e){let n=l.get(e.pointerId);if(n===void 0)return;if(l.set(e.pointerId,{clientX:e.clientX,clientY:e.clientY}),l.size===2){let e=p(),n=u/e;t=o(t*n),u=e;return}if(l.size!==1)return;let r=e.clientX-n.clientX,i=e.clientY-n.clientY;s=r,c=i,a(r,i)}function g(e){l.delete(e.pointerId)}function x(e){l.delete(e.pointerId)}function S(e){e.preventDefault(),t=o(t*(1+e.deltaY*v))}return e.addEventListener(`pointerdown`,m),window.addEventListener(`pointermove`,h),window.addEventListener(`pointerup`,g),window.addEventListener(`pointercancel`,x),e.addEventListener(`wheel`,S,{passive:!1}),{tick(){if(!(l.size>0)){if(Math.abs(s)<.1&&Math.abs(c)<.1){s=0,c=0;return}a(s,c),s*=b,c*=b}},getViewMatrix(){let e=d.scale(r,t),n=d.fromValues(0,0,0);return f.lookAt(e,n,i)},destroy(){e.removeEventListener(`pointerdown`,m),window.removeEventListener(`pointermove`,h),window.removeEventListener(`pointerup`,g),window.removeEventListener(`pointercancel`,x),e.removeEventListener(`wheel`,S)}}}function O(e){let t=!1,n,r=D(e);return k(e,r).then(e=>{t?e():n=e}),()=>{t=!0,r.destroy(),n?.()}}async function k(e,t){let n=await i(e),o=r(4),s=new a([new T(t,o)]);s.initAll(n);let c=E({canvas:e,context:n,layerManager:s});return()=>{c(),s.dispose(),o.dispose(),n.device.destroy()}}var A=e(),j=(0,p.memo)(()=>{let e=(0,p.useRef)(null);return(0,p.useEffect)(()=>{if(e.current)return O(e.current)},[]),(0,A.jsx)(c,{className:`h-full w-full`,children:(0,A.jsx)(`canvas`,{ref:e,className:`h-full w-full [touch-action:none]`})})});export{j as Sun};