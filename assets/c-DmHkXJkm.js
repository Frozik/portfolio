import{F as e,N as t,P as n,ct as r}from"./c-DPkLgPic.js";import{i,n as a,r as o,t as s}from"./c-Czl7OTz9.js";import{i as c,n as l,r as u,t as d}from"./c-BBEFB0xa.js";import{t as f}from"./c-wJEgkeeX.js";var p=r(n(),1),m=`const PI: f32 = 3.1415926535;
...
`,h=1e5,g={r:.149,g:.149,b:.149,a:1},_=.005,v=.01,y=Math.PI/2;Math.PI-.01;var b=.95,x=Math.PI/4,S=.1,C=1e3,w=`depth24plus`,T=1,E=class{device;format;pipeline;bindGroup;uniformBuffer;uniformView;depthTexture=null;constructor(e,t){this.camera=e,this.msaaManager=t}init(e){this.device=e.device,this.format=e.format,this.uniformView=l(d(m).uniforms.U),this.uniformBuffer=this.device.createBuffer({size:this.uniformView.arrayBuffer.byteLength,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});let t=this.device.createShaderModule({code:m}),n=this.device.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:`uniform`}}]});this.pipeline=this.device.createRenderPipeline({layout:this.device.createPipelineLayout({bindGroupLayouts:[n]}),vertex:{module:t,entryPoint:`vs`},fragment:{module:t,entryPoint:`fs`,targets:[{format:this.format}]},primitive:{topology:`triangle-list`,cullMode:`none`},depthStencil:{depthWriteEnabled:!0,depthCompare:`less`,format:w},multisample:{count:4}}),this.bindGroup=this.device.createBindGroup({layout:n,entries:[{binding:0,resource:{buffer:this.uniformBuffer}}]})}update(e){this.camera.tick();let t=this.camera.getViewMatrix(),n=e.canvasWidth/Math.max(T,e.canvasHeight),r=s.perspective(x,n,S,100),i=s.multiply(r,t);this.uniformView.set({time:e.time,mvp:i}),this.device.queue.writeBuffer(this.uniformBuffer,0,this.uniformView.arrayBuffer)}render(t,n,r){let i=this.msaaManager.ensureView(this.device,this.format,r.canvasWidth,r.canvasHeight);if(e(i))return;let a=this.ensureDepthTexture(r.canvasWidth,r.canvasHeight),o=t.beginRenderPass({colorAttachments:[{view:i,resolveTarget:n,loadOp:`clear`,clearValue:g,storeOp:`discard`}],depthStencilAttachment:{view:a.createView(),depthClearValue:1,depthLoadOp:`clear`,depthStoreOp:`discard`}});o.setPipeline(this.pipeline),o.setBindGroup(0,this.bindGroup),o.draw(3,h,0,0),o.end()}dispose(){this.uniformBuffer.destroy(),this.depthTexture?.destroy()}ensureDepthTexture(t,n){return!e(this.depthTexture)&&this.depthTexture.width===t&&this.depthTexture.height===n?this.depthTexture:(this.depthTexture?.destroy(),this.depthTexture=this.device.createTexture({size:[Math.max(T,t),Math.max(T,n)],format:w,sampleCount:4,usage:GPUTextureUsage.RENDER_ATTACHMENT}),this.depthTexture)}};function D(e){let{canvas:t,context:n,layerManager:r}=e,{device:i,canvasContext:a}=n,o=0,s=0,c=Math.max(1,window.devicePixelRatio);function l(){c=Math.max(1,window.devicePixelRatio);let e=Math.floor(t.clientWidth*c),n=Math.floor(t.clientHeight*c);(t.width!==e||t.height!==n)&&(t.width=e,t.height=n),o=e,s=n}l();let u=new ResizeObserver(()=>{l()});u.observe(t);let d=0,f=!1,p=performance.now();function m(){if(f)return;if(l(),o===0||s===0){d=requestAnimationFrame(m);return}let e={time:(performance.now()-p)/C,canvasWidth:o,canvasHeight:s,devicePixelRatio:c};r.updateAll(e);let t=a.getCurrentTexture().createView(),n=i.createCommandEncoder();r.renderAll(n,t,e),i.queue.submit([n.finish()]),d=requestAnimationFrame(m)}return d=requestAnimationFrame(m),()=>{f=!0,cancelAnimationFrame(d),u.disconnect()}}function O(e){let t=16,n=y,r=a.fromValues(Math.sin(n)*Math.sin(0),Math.cos(n),Math.sin(n)*Math.cos(0)),i=a.fromValues(0,1,0);function o(e,t){let n=a.negate(r),o=a.normalize(a.cross(n,i));if(Math.abs(t)>0){let e=-t*_,n=s.rotation(o,e);r=a.transformMat4(r,n),i=a.normalize(a.transformMat4(i,n))}if(Math.abs(e)>0){let t=-e*_,n=s.rotation(i,t);r=a.transformMat4(r,n)}r=a.normalize(r)}let c=0,l=0,u=!1,d=0,f=0;function p(e){u=!0,d=e.clientX,f=e.clientY}function m(e){if(!u)return;let t=e.clientX-d,n=e.clientY-f;d=e.clientX,f=e.clientY,c=t,l=n,o(t,n)}function h(){u=!1}function g(e){e.preventDefault(),t=Math.max(5,Math.min(20,t*(1+e.deltaY*v)))}e.addEventListener(`mousedown`,p),window.addEventListener(`mousemove`,m),window.addEventListener(`mouseup`,h),e.addEventListener(`wheel`,g,{passive:!1});let x=0,S=0,C=!1,w=0;function T(e){let t=e.touches[0].clientX-e.touches[1].clientX,n=e.touches[0].clientY-e.touches[1].clientY;return Math.sqrt(t*t+n*n)}function E(e){e.touches.length===1?(C=!0,x=e.touches[0].clientX,S=e.touches[0].clientY):e.touches.length===2&&(C=!1,w=T(e))}function D(e){if(e.preventDefault(),e.touches.length===2){let n=T(e),r=w/n;t=Math.max(5,Math.min(20,t*r)),w=n;return}if(!C||e.touches.length!==1)return;let n=e.touches[0].clientX-x,r=e.touches[0].clientY-S;x=e.touches[0].clientX,S=e.touches[0].clientY,c=n,l=r,o(n,r)}function O(e){e.touches.length===0?C=!1:e.touches.length===1&&(C=!0,x=e.touches[0].clientX,S=e.touches[0].clientY)}return e.addEventListener(`touchstart`,E,{passive:!0}),e.addEventListener(`touchmove`,D,{passive:!1}),e.addEventListener(`touchend`,O),{tick(){if(!(u||C)){if(Math.abs(c)<.1&&Math.abs(l)<.1){c=0,l=0;return}o(c,l),c*=b,l*=b}},getViewMatrix(){let e=a.scale(r,t),n=a.fromValues(0,0,0);return s.lookAt(e,n,i)},destroy(){e.removeEventListener(`mousedown`,p),window.removeEventListener(`mousemove`,m),window.removeEventListener(`mouseup`,h),e.removeEventListener(`wheel`,g),e.removeEventListener(`touchstart`,E),e.removeEventListener(`touchmove`,D),e.removeEventListener(`touchend`,O)}}}function k(e){let t=!1,n,r=O(e);return A(e,r).then(e=>{t?e():n=e}),()=>{t=!0,r.destroy(),n?.()}}async function A(e,t){let n=await i(e),r=c(4),a=new o([new E(t,r)]);a.initAll(n);let s=D({canvas:e,context:n,layerManager:a});return()=>{s(),a.dispose(),r.dispose(),n.device.destroy()}}var j=t(),M=(0,p.memo)(()=>{let e=(0,p.useRef)(null);return(0,p.useEffect)(()=>{if(e.current)return k(e.current)},[]),(0,j.jsx)(u,{className:f.fixedContainer,children:(0,j.jsx)(`div`,{className:f.fixedContainer,children:(0,j.jsx)(`canvas`,{ref:e,style:{width:`100%`,height:`100%`}})})})});export{M as Sun};