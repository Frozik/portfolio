import{G as e,K as t,L as n,bt as r,i,o as a,q as o,r as s}from"./c-lahOAW2I.js";import{t as c}from"./c-rc86jWN7.js";import{t as l}from"./c-BdIG0Mup.js";import{n as u,t as d}from"./c-Br0-WPhW.js";import{t as f}from"./c-K9g4nGQa.js";var p=r(t(),1),m=.149,h=.149,g=.149,_=.5,v=.7,y=1e-4,b=16384,x=.5,S=.4,C=.6,w=`rgba8unorm`;function T(e){return Math.trunc(e/20/4)*4+1}function E(e){return Math.trunc(e/20/4)*4+1}var D={color:{srcFactor:`src-alpha`,dstFactor:`one-minus-src-alpha`,operation:`add`},alpha:{srcFactor:`one`,dstFactor:`one-minus-src-alpha`,operation:`add`}};function O(e,t,n){let r=null,i=null,a=null,s=null,c=null;return{ensureOffscreenTextures(l,u,d,f,p){return!o(r)&&r.width===l&&r.height===u?o(i)||o(s)||o(c)?null:{offscreenMsaaView:i,offscreenResolveView:s,compositeBindGroup:c}:(r?.destroy(),a?.destroy(),l===0||u===0?(r=null,i=null,a=null,s=null,c=null,null):(r=e.createTexture({size:[l,u],format:t,sampleCount:n,usage:GPUTextureUsage.RENDER_ATTACHMENT}),i=r.createView(),a=e.createTexture({size:[l,u],format:t,sampleCount:1,usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.TEXTURE_BINDING}),s=a.createView(),c=e.createBindGroup({layout:d,entries:[{binding:0,resource:s},{binding:1,resource:f},{binding:2,resource:{buffer:p}}]}),{offscreenMsaaView:i,offscreenResolveView:s,compositeBindGroup:c}))},destroy(){r?.destroy(),a?.destroy()}}}var k=`struct CompositeUniforms {
    opacity: f32,
};
...
`,A=3,j=16;function M(e){let t=e.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:`float`}},{binding:1,visibility:GPUShaderStage.FRAGMENT,sampler:{type:`filtering`}},{binding:2,visibility:GPUShaderStage.FRAGMENT,buffer:{type:`uniform`}}]}),n=e.createSampler({magFilter:`linear`,minFilter:`linear`}),r=e.createBuffer({size:j,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),i=new Float32Array([v,0,0,0]);return e.queue.writeBuffer(r,0,i),{compositeBindGroupLayout:t,compositeSampler:n,compositeUniformBuffer:r}}var N=class{compositePipeline;constructor(e,t){this.textureManager=e,this.resources=t}init(e){let{device:t,format:n}=e,r=t.createShaderModule({code:k});this.compositePipeline=t.createRenderPipeline({layout:t.createPipelineLayout({bindGroupLayouts:[this.resources.compositeBindGroupLayout]}),vertex:{module:r,entryPoint:`vsComposite`},fragment:{module:r,entryPoint:`fsComposite`,targets:[{format:n,blend:D}]},primitive:{topology:`triangle-list`}})}update(e){}render(e,t,n){let r=this.textureManager.ensureOffscreenTextures(n.canvasWidth,n.canvasHeight,this.resources.compositeBindGroupLayout,this.resources.compositeSampler,this.resources.compositeUniformBuffer);if(o(r))return;let i=e.beginRenderPass({colorAttachments:[{view:t,loadOp:`load`,storeOp:`store`}]});i.setPipeline(this.compositePipeline),i.setBindGroup(0,r.compositeBindGroup),i.draw(A,1,0,0),i.end()}dispose(){this.resources.compositeUniformBuffer.destroy()}},P=`struct Uniforms {
    mvp: mat4x4<f32>,
    viewport: vec2<f32>,
    time: f32,
    sinCount: u32,
...
`;function F(e,t){let n=u(d(t).uniforms.U),r=e.createBuffer({size:n.arrayBuffer.byteLength,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});return{buffer:r,writeFromFrameState(e,t){let i=t.canvasWidth*_,a=t.canvasHeight*_,o=f.ortho(-i,i,-a,a,-1,1),s=T(t.canvasWidth),c=E(t.canvasHeight);n.set({mvp:o,viewport:[t.canvasWidth,t.canvasHeight],time:t.time,sinCount:s,sinPenMin:2,sinPenMax:20,borderMargin:20,borderOffset:s,sinYCount:c}),e.queue.writeBuffer(r,0,n.arrayBuffer)},dispose(){r.destroy()}}}var I=class{device;format;uniformManager;pipeline;bindGroup;constructor(e,t){this.chartShaderModule=e,this.msaaManager=t}init(e){this.device=e.device,this.format=e.format,this.uniformManager=F(this.device,P);let t=this.device.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:`uniform`}}]});this.pipeline=this.device.createRenderPipeline({layout:this.device.createPipelineLayout({bindGroupLayouts:[t]}),vertex:{module:this.chartShaderModule,entryPoint:`vs`},fragment:{module:this.chartShaderModule,entryPoint:`fs`,targets:[{format:this.format,blend:D}]},primitive:{topology:`triangle-list`},multisample:{count:4}}),this.bindGroup=this.device.createBindGroup({layout:t,entries:[{binding:0,resource:{buffer:this.uniformManager.buffer}}]})}update(e){this.uniformManager.writeFromFrameState(this.device,e)}render(e,t,n){let r=T(n.canvasWidth)+4,i=this.msaaManager.ensureView(this.device,this.format,n.canvasWidth,n.canvasHeight);if(o(i))return;let a=e.beginRenderPass({colorAttachments:[{view:i,resolveTarget:t,loadOp:`clear`,clearValue:{r:m,g:h,b:g,a:1},storeOp:`discard`}]});a.setPipeline(this.pipeline),a.setBindGroup(0,this.bindGroup),r>0&&a.draw(18,r,0,0),a.end()}dispose(){this.uniformManager.dispose()}};function L(e,t){return e+Math.random()*(t-e)}function R(e){let t=Math.random(),n=Math.random(),r=Math.random(),i=(t+n+r)/3;if(i<.4){let e=S/Math.max(i,.01);t=Math.min(1,t*e),n=Math.min(1,n*e),r=Math.min(1,r*e)}return{x:0,y:0,halfSize:L(40/2,160/2),spawnTime:e,r:t,g:n,b:r,holdDuration:L(2,3),shapeType:Math.floor(Math.random()*10),fillMode:Math.random()<.5?0:1,maxOpacity:L(C,1)}}function z(e){return 2*x+e.holdDuration}function B(e,t,n){t[n]=e.x,t[n+1]=e.y,t[n+2]=e.halfSize,t[n+3]=e.spawnTime,t[n+4]=e.r,t[n+4+1]=e.g,t[n+4+2]=e.b,t[n+4+3]=e.holdDuration,t[n+8]=e.shapeType,t[n+8+1]=e.fillMode,t[n+8+2]=e.maxOpacity,t[n+8+3]=0}function V(e,t,n){let r=e/n*(t/n);return Math.min(Math.max(1,Math.round(r*y)),b)}function H(e){let t=[],n=2*x+5/2;for(let r=0;r<e;r++){let i=R(0);i.spawnTime=-(n/e)*r,t.push(i)}return t}function U(e,t,n,r,i){if(t>e.length){let a=2*x+5/2,o=t-e.length;for(let t=0;t<o;t++){let s=R(n);s.spawnTime=n-a/o*t,s.x=L(-r+s.halfSize,r-s.halfSize),s.y=L(-i+s.halfSize,i-s.halfSize),e.push(s)}}else t<e.length&&e.splice(t)}function W(e){return new Float32Array(e*48/Float32Array.BYTES_PER_ELEMENT)}var G=P+`const HALF: f32 = 0.5;
...
`,K=48/Float32Array.BYTES_PER_ELEMENT,q=class{device;uniformManager;shapesPipeline;shapesBindGroup;shapesStorageBuffer;shapeDataBuffer=W(b);shapes=[];init(e){this.device=e.device,this.uniformManager=F(this.device,G);let t=this.device.createShaderModule({code:G}),n=this.device.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:`uniform`}},{binding:1,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:`read-only-storage`}}]});this.shapesStorageBuffer=this.device.createBuffer({size:b*48,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.shapesPipeline=this.device.createRenderPipeline({layout:this.device.createPipelineLayout({bindGroupLayouts:[n]}),vertex:{module:t,entryPoint:`vsShapes`},fragment:{module:t,entryPoint:`fsShapes`,targets:[{format:e.format,blend:{color:{srcFactor:`one`,dstFactor:`one-minus-src-alpha`,operation:`add`},alpha:{srcFactor:`one`,dstFactor:`one-minus-src-alpha`,operation:`add`}}}]},primitive:{topology:`triangle-list`}}),this.shapesBindGroup=this.device.createBindGroup({layout:n,entries:[{binding:0,resource:{buffer:this.uniformManager.buffer}},{binding:1,resource:{buffer:this.shapesStorageBuffer}}]})}update(e){this.uniformManager.writeFromFrameState(this.device,e);let{time:t,canvasWidth:n,canvasHeight:r,devicePixelRatio:i}=e,a=n*_,o=r*_;this.shapes.length===0&&(this.shapes=H(V(n,r,i)));let s=V(n,r,i);s!==this.shapes.length&&U(this.shapes,s,t,a,o);for(let e=0;e<this.shapes.length;e++){let n=this.shapes[e];if(t-n.spawnTime>z(n)){let n=R(t);n.x=L(-a+n.halfSize,a-n.halfSize),n.y=L(-o+n.halfSize,o-n.halfSize),this.shapes[e]=n}B(this.shapes[e],this.shapeDataBuffer,e*K)}this.device.queue.writeBuffer(this.shapesStorageBuffer,0,this.shapeDataBuffer.buffer,0,this.shapes.length*48)}render(e,t,n){if(this.shapes.length===0)return;let r=e.beginRenderPass({colorAttachments:[{view:t,loadOp:`load`,storeOp:`store`}]});r.setPipeline(this.shapesPipeline),r.setBindGroup(0,this.shapesBindGroup),r.draw(6,this.shapes.length,0,0),r.end()}dispose(){this.uniformManager.dispose(),this.shapesStorageBuffer.destroy()}},J=class{device;uniformManager;sinYPipeline;bindGroup;constructor(e,t,n,r,i){this.textureManager=e,this.compositeBindGroupLayout=t,this.compositeSampler=n,this.compositeUniformBuffer=r,this.chartShaderModule=i}init(e){this.device=e.device,this.uniformManager=F(this.device,P);let t=this.device.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:`uniform`}}]});this.sinYPipeline=this.device.createRenderPipeline({layout:this.device.createPipelineLayout({bindGroupLayouts:[t]}),vertex:{module:this.chartShaderModule,entryPoint:`vsSinY`},fragment:{module:this.chartShaderModule,entryPoint:`fsSinY`,targets:[{format:w,blend:D}]},primitive:{topology:`triangle-list`},multisample:{count:4}}),this.bindGroup=this.device.createBindGroup({layout:t,entries:[{binding:0,resource:{buffer:this.uniformManager.buffer}}]})}update(e){this.uniformManager.writeFromFrameState(this.device,e)}render(e,t,n){let r=E(n.canvasHeight),i=this.textureManager.ensureOffscreenTextures(n.canvasWidth,n.canvasHeight,this.compositeBindGroupLayout,this.compositeSampler,this.compositeUniformBuffer);if(o(i))return;let a=e.beginRenderPass({colorAttachments:[{view:i.offscreenMsaaView,resolveTarget:i.offscreenResolveView,loadOp:`clear`,clearValue:{r:0,g:0,b:0,a:0},storeOp:`discard`}]});a.setPipeline(this.sinYPipeline),a.setBindGroup(0,this.bindGroup),r>0&&a.draw(18,r,0,0),a.end()}dispose(){this.uniformManager.dispose()}};function Y(e){let{canvas:t,context:r,layerManager:i}=e,{device:a,canvasContext:o}=r,s=0,c=0,l=Math.max(1,window.devicePixelRatio);function u(){l=Math.max(1,window.devicePixelRatio);let e=Math.floor(t.clientWidth*l),n=Math.floor(t.clientHeight*l);(t.width!==e||t.height!==n)&&(t.width=e,t.height=n),s=e,c=n}u();let d=new ResizeObserver(()=>{u()});d.observe(t);let f=0,p=!1,m=performance.now();function h(){if(p)return;if(u(),s===0||c===0){f=requestAnimationFrame(h);return}let e={time:(performance.now()-m)/n,canvasWidth:s,canvasHeight:c,devicePixelRatio:l};i.updateAll(e);let t=o.getCurrentTexture().createView(),r=a.createCommandEncoder();i.renderAll(r,t,e),a.queue.submit([r.finish()]),f=requestAnimationFrame(h)}return f=requestAnimationFrame(h),()=>{p=!0,cancelAnimationFrame(f),d.disconnect()}}var X=P+`const PI: f32 = 3.14159265358979323846;
const HALF: f32 = 0.5;
...
`;function Z(e){let t=!1,n;return Q(e).then(e=>{t?e():n=e}),()=>{t=!0,n?.()}}async function Q(e){let t=await a(e),{device:n}=t,r=n.createShaderModule({code:X}),o=O(n,w,4),c=i(4),l=M(n),u=new s([new I(r,c),new J(o,l.compositeBindGroupLayout,l.compositeSampler,l.compositeUniformBuffer,r),new N(o,l),new q]);u.initAll(t);let d=Y({canvas:e,context:t,layerManager:u});return()=>{d(),u.dispose(),c.dispose(),o.destroy(),n.destroy()}}var $=e(),ee=(0,p.memo)(()=>{let e=(0,p.useRef)(null);return(0,p.useEffect)(()=>{if(e.current)return Z(e.current)},[]),(0,$.jsx)(c,{className:l.fixedContainer,children:(0,$.jsx)(`div`,{className:l.fixedContainer,children:(0,$.jsx)(`canvas`,{ref:e,className:`h-full w-full`})})})});export{ee as Charts};